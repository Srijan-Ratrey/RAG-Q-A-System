...
class RAGEvaluator:
    ...
    def _evaluate_response_quality(
        self, question: TestQuestion, response: Dict[str, Any]
    ) -> Dict[str, float]:
        ...
        haystack = (
            answer + " " + " ".join(c.get("text", "") for c in contexts)
        ).lower()

        # Count how many expected keywords appear anywhere in the haystack.
        expected_matches = sum(
            1 for expected in question.expected_sources if expected.lower() in haystack
        )
    ...
    def _compare_responses(
        self, baseline: Dict[str, Any], hybrid: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def evaluate_question(self, question: TestQuestion, k: int = 5) -> EvaluationResult:
        ...
        metrics = {
            ...
            "performance": {"baseline_time": baseline_time, "hybrid_time": hybrid_time},
        }
    ...
    def _calculate_overall_stats(
        ...
        baseline_avg_conf = sum(
            r.baseline_response.get("confidence", 0.0) for r in results
        ) / len(results)
        hybrid_avg_conf = sum(
            r.hybrid_response.get("confidence", 0.0) for r in results
        ) / len(results)
        ...
        return {
            ...
            "confidence_improvements": {
                ...
                "average_improvement": sum(confidence_improvements)
    ...
    def _calculate_category_stats(
        self, category_results: List[EvaluationResult]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "answer_rate": {
                "baseline": sum(
                    1 for r in category_results if r.baseline_response.get("answer")
                )
                / len(category_results),
                "hybrid": sum(
                    1 for r in category_results if r.hybrid_response.get("answer")
                )
                / len(category_results),
            },
            "avg_confidence": {
                "baseline": sum(
                    r.baseline_response.get("confidence", 0.0) for r in category_results
                )
                / len(category_results),
                "hybrid": sum(
                    r.hybrid_response.get("confidence", 0.0) for r in category_results
                )
                / len(category_results),
            },
        }
    ...
    def print_evaluation_summary(self, report: Dict[str, Any]) -> None:
        ...
        for category, stats in report["category_breakdown"].items():
            ...
            print(
                f"    Answer rate: {stats['answer_rate']['baseline']:.1%} → {stats['answer_rate']['hybrid']:.1%}"
            )
            print(
                f"    Confidence:  {stats['avg_confidence']['baseline']:.3f} → {stats['avg_confidence']['hybrid']:.3f}"
            )
        ...
        if overall["answer_rates"]["improvement"] > 0:
            print(
                f"  ✅ Hybrid reranker improved answer rate by {overall['answer_rates']['improvement']:.1%}"
            )
        elif overall["answer_rates"]["improvement"] < 0:
            print(
                f"  ⚠️ Hybrid reranker decreased answer rate by {abs(overall['answer_rates']['improvement']):.1%}"
            )
        else:
        ...
        if overall["average_confidence"]["improvement"] > 0.05:
            print(
                f"  ✅ Significant confidence improvement (+{overall['average_confidence']['improvement']:.3f})"
            )
        elif overall["average_confidence"]["improvement"] < -0.05:
            print(
                f"  ⚠️ Confidence decreased (-{abs(overall['average_confidence']['improvement']):.3f})"
            )
        else:
            print(
                f"  ➡️ Minimal confidence change ({overall['average_confidence']['improvement']:+.3f})"
            )

        if overall["ranking_changes"]["percentage"] > 0.5:
            print(
                f"  🔄 High reranking activity ({overall['ranking_changes']['percentage']:.1%} of questions)"
            )
        else:
            print(
                f"  🔄 Moderate reranking activity ({overall['ranking_changes']['percentage']:.1%} of questions)"
            )
