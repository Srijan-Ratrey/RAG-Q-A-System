...
def _build(chunk_db, ordered, stale=False):
    ...
    return SearchSystem(
        db_path=db_path, embedding_system=fake, similarity_threshold=0.0
    )
...
def test_bm25_scores_only_matching_candidates(chunk_db):
    _, rows = chunk_db
    ordered = [
        _chunk_info(r["chunk_id"], r["text"], r["source_file"], 0.9) for r in rows
    ]
    ss = _build(chunk_db, ordered)
    scores = ss._get_bm25_scores_for_chunks(
        "emergency stop", [r["chunk_id"] for r in rows]
    )
...
def test_hybrid_confidence_is_vector_score(chunk_db):
    ...
    _, rows = chunk_db
    ordered = [
        _chunk_info(r["chunk_id"], r["text"], r["source_file"], 0.75) for r in rows
    ]
    ss = _build(chunk_db, ordered)
